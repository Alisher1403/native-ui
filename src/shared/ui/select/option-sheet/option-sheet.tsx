...
import BottomSheetHeader from "@src/shared/ui/bottom-sheet/header";
  ...
  return (
    <View style={styles.emptyContainer}>
      <FastImage
        ...
        resizeMode="contain"
      />
      <Typography name="body/semibold" color="main/label" align="center" mt={16}>
        {title}
      </Typography>
    </View>
  );
    ...
    <BottomSheet
      ...
      headerComponent={
        props.title || props.label ? (
          <BottomSheetHeader title={props.title || props.label} onClosePress={() => ref.current?.close()} />
        ) : undefined
      }
    >
      {isSearchVisible ? <OptionSheetSearch search={search} setSearch={setSearch} /> : null}
      ...
      <BottomSheetFlatList
        ...
        ListEmptyComponent={loading ? <LoadingComponent /> : <EmptyComponent title={props.listEmptyTitle} />}
        ...
        renderItem={({ item }: any) => {
          ...
          return (
            <TouchableOpacity style={styles.option} onPress={() => onSelect(item.value)}>
              ...
              <Division>{isActive ? <Icon name="check-line" size={24} color="main/primary" /> : undefined}</Division>
            </TouchableOpacity>
          );
        }}
      />
